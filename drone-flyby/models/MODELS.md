...
| 4 | `ta-ta` | 6.7 x 3.6 | 25 | 0.758 | four-legged walker (AT-AT) | **fitted: AT-AT** (`models/ta-ta/atat/`, OBJ, `--up y`), IoU 0.78; licence line still to copy from CGTrader |
| 5 | `large_tower` | 13.9 x 12.6 | 19 | 0.771 | brown/camo rectangular structure, pale band across the middle | **fitted: CGTrader lattice tower**, IoU 0.75 with `--close-gaps 20 --tilts 0 8 16 24 32 40 --thicken 2` (beams twice as wide, Juan: the real legs are thicker): wooden legs with cross planks, ladder, cabin. The cut-outs fill the ground between the legs, so the fit fills the render's outline the same way; it sits near the frame corner, seen ~32-40 deg off vertical (the real one may be ~25% taller) |
...
| 9 | `condor` | 36.3 x 34.9 | 11 (3 suspect) | 1.000 | big grey X-shaped four-engine aircraft/cargo drone | **fitted: AS-78 Condor** (`models/condor/as78/`, OBJ, `--up y --drop Cylinder.002_Cylinder.003_39`, a studio backdrop box), IoU 0.83; licence line still to copy from CGTrader. Two cut-outs (f007, f008) include a patch of sand |
...
| 12 | `helicopter` | 24.4 x 19.7 | 19 (4 suspect) | 1.000 | dark green helicopter, rotors visible | **downloaded + fitted** — Mi-28N, IoU 0.55 with `--up=-z --drop rotor_Body` (with `--up z` it fitted upside down: the top-down outline is the same): fuselage pose right, the real rotor turns between frames, so add it back at random angles when generating |
...
| `large_tower` | [Old wooden watchtower (low poly)](https://www.cgtrader.com/free-3d-models/exterior/other/old-wooden-watchtower-low-poly) | CGTrader | free — **check the licence line** | downloaded (`models/large_tower/cgtrader_watchtower/`, OBJ) — **used**; Juan confirmed its legs match the real tower |
...
| `ta-ta` | [Star Wars AT-AT Walker](https://www.cgtrader.com/free-3d-models/space/other/star-wars-at-at-walker-68bd9d4c-a316-4517-8b6c-b6feb3b1da77) | CGTrader | free — **check the licence line** | downloaded — 32x17 px on screen, so the silhouette of the legs is all that matters. Star Wars IP, same grey area as the TIE |
...
| `condor` | [Battlefield 2042 Condor](https://www.cgtrader.com/free-3d-models/military/military-vehicle/battlefield-2042-condor-flight) | CGTrader | free — **check the licence line**, and see the game-asset caveat | downloaded — quad-rotor VTOL, matches the X-shaped 4-engine sprite |
