...
| 5 | `large_tower` | 13.9 x 12.6 | 19 | 0.771 | brown/camo rectangular structure, pale band across the middle | **fitted: CGTrader lattice tower**, IoU 0.75 with `--close-gaps 20 --tilts 0 8 16 24 32 40`: wooden legs with cross planks, ladder, cabin. The cut-outs fill the ground between the legs, so the fit fills the render's outline the same way; it sits near the frame corner, seen ~32-40 deg off vertical (the real one may be ~25% taller) |
