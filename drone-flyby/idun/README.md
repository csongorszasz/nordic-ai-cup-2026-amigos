...
versus the 6 GB on Juan's laptop. It cannot serve the endpoint (compute nodes are not
reachable from the internet), so the loop is: **train on IDUN, fetch the weights, serve locally.**

Adapted from Juan's thesis harness (`thesis-prep/src/idun/`), which is known to work there.
...
```bash
bash idun/submit.sh train-synth 300        # synthetic dataset (300 frames + Helsinki) and a 60-epoch YOLO run
bash idun/submit.sh run python training/train_yolo.py --epochs 60 --batch 32 --name my_run
...
bash idun/submit.sh fetch                  # copy runs/ back: runs/<name>/weights/best.pt
```

Then serve the new weights locally:

```bash
DRONE_MODEL=runs/synth_300/weights/best.pt .venv/bin/python -m uvicorn api:app --app-dir src --port 9056
```
...
|---|---|
| `src/`, `training/`, `idun/`, `requirements.txt` | `.venv/` (IDUN has its own env) |
| `data/helsinki/` (~400 MB, first sync only) | `datasets/` (rebuilt on IDUN, faster than uploading) |
| `sprites/`, `backgrounds/`, `models/` | `runs/` (comes back with `fetch`), `recordings/`, `*.pt` |

`yolo11s.pt` (the pretrained starting point) is downloaded on IDUN by `setup`, not uploaded:
it is gitignored, so a teammate's checkout would not have it.
...
**Before the first `train-synth`**, the laptop needs `sprites/` (`bash training/fetch_data.sh
model`) and `backgrounds/` (`python training/fetch_backgrounds.py`); both are uploaded from here.
